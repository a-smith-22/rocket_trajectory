...
print('  Stage 1 (MR): ' + str("{:.4f}".format( O50SXL_W[0]/O50SXL_W[5] )) ) # baseline mass fraction (mo/mf) -> use as minimum for stage mass fraction
...
# Mass parameters
# mass flow rates
dm_1 = O50SXL_W[1] / O50SXL_P[0] # stage 1 mass flow rate, assume constant -> dm = propellent mass / burn time
...
for mr_1 in MR_1:
    for mr_2 in MR_2:
        for mr_3 in MR_3: # iterate over each stage mass fraction
            ...
            m_s1_f = O50SXL_W[1]/(mr_1 - 1)    # stage one final (dry) mass   -> for MR = mf/mo, mf = mo*MR = 1/(MR-1) * mp
            m_s1_o = m_s1_f * mr_1             # stage one initial (wet) mass -> for MR = mf/mo, mo = mf/MR = MR/(MR-1) * mf
            ...
            MRo_vals.append( m0 / pl )           # overall vehicle mass fraction -> intial mass / final mass
            ...
            while (t <= t_max):
                ...
                if stage == 2:
                    m = m0 - m_s1_o - m_dot*(t - O50SXL_P[0])                           # " " + subtract above stage burnout mass
                if stage == 3:
                    m = m0 - (m_s1_o + m_s2_o) - m_dot*(t - (O50SXL_P[0] + O50XL_P[0])) # " "
                if t > tb_total:
                    m = pl                                                              # subtract all stage burnout masses
...
print("  Trajectory Range:     " + str("{:.3f}".format( min_alt/1000 )) + "-" + str("{:.3f}".format( max_alt/1000 )) + "km" )
print("")
...
print("  Apogee time:          " + str("{:.3f}".format( t_at_max[index] )) + "s")                                                                                                         # time to reach above altitude
...
print("  Stage 1 delta-V: " + str("{:.3f}".format( s1_vf[index]/1000 )) + "km/s")                  # compute delta V (no initial velocity)
print("  Stage 2 delta-V: " + str("{:.3f}".format( (s2_vf[index] - s1_vf[index])/1000 )) + "km/s") # " " use previous stage burnout velocity as initial velocity
print("  Stage 3 delta-V: " + str("{:.3f}".format( (s3_vf[index] - s2_vf[index])/1000 )) + "km/s") # " "
